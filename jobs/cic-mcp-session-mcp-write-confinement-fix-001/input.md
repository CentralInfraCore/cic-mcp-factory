# Job: cic-mcp-session-mcp-write-confinement-fix-001
...
(SHA256-egyezés ellenőrizve) `cic-mcp-knowledge`/`cic-mcp-session`/
...
repóban (`cic-mcp-knowledge`, `cic-mcp-shared`, `cic-mcp-gateway`) PÁRHUZAMOS,
KÜLÖN jobok futnak ugyanezzel a logikával (`cic-mcp-knowledge-mcp-write-
...
- target path: `output/cic-mcp-session-mcp-write-confinement-fix.md` +
...
Cseréld ki `metadata.name: base`-t a repo TÉNYLEGES nevére (`cic-mcp-
session`). NE módosíts más mezőt (`description`/`tags`/`version`/`license`/
...
- a másik 3 repó (`cic-mcp-knowledge`/`cic-mcp-shared`/`cic-mcp-gateway`)
...
- `output/cic-mcp-session-mcp-write-confinement-fix.md`
...
Push a `feature/cic-mcp-session-mcp-write-confinement-fix-001` branch-re,
