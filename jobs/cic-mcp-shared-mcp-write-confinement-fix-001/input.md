# Job: cic-mcp-shared-mcp-write-confinement-fix-001
...
repóban (`cic-mcp-session`, `cic-mcp-knowledge`, `cic-mcp-gateway`) PÁRHUZAMOS,
...
- target path: `output/cic-mcp-shared-mcp-write-confinement-fix.md` +
...
Cseréld ki `metadata.name: base`-t a repo TÉNYLEGES nevére (`cic-mcp-shared`). NE módosíts más mezőt (`description`/`tags`/`version`/`license`/
...
- a másik 3 repó (`cic-mcp-session`/`cic-mcp-knowledge`/`cic-mcp-gateway`)
...
- `output/cic-mcp-shared-mcp-write-confinement-fix.md`
...
Push a `feature/cic-mcp-shared-mcp-write-confinement-fix-001` branch-re,
