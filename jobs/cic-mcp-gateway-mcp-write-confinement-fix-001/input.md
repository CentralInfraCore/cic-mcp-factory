# Job: cic-mcp-gateway-mcp-write-confinement-fix-001
...
repóban (`cic-mcp-session`, `cic-mcp-knowledge`, `cic-mcp-shared`) PÁRHUZAMOS,
...
- target path: `output/cic-mcp-gateway-mcp-write-confinement-fix.md` +
...
Cseréld ki `metadata.name: base`-t a repo TÉNYLEGES nevére (`cic-mcp-gateway`). NE módosíts más mezőt (`description`/`tags`/`version`/`license`/
`owner`/`validatedBy` MARADJON érintetlen — ez egy KÜLÖN, szélesebb döntés,
NEM ennek a jobnak a hatóköre).

### 5. `.mcp.json.tpl` hiányzó `cic-gateway` bejegyzés

A `gateway-context-pack-production-wiring-001` (mergelve) megépítette
`mcp-server/gateway_server.py`-t (`FastMCP("cic-gateway")`,
`get_gateway_context_pack` tool) — DE a `.mcp.json.tpl` (amiből `make
mcp.config` egy friss clone-nak generálja a saját `.mcp.json`-ját) MÁIG
KIZÁRÓLAG a generikus `cic-graph` bejegyzést tartalmazza. Egy friss
`cic-mcp-gateway` clone tehát NEM kapja meg automatikusan a `cic-gateway`
MCP szerver bekötését.

```
cat .mcp.json.tpl
```

Idézd a kimenetet. Adj hozzá egy ÚJ `"cic-gateway"` bejegyzést, a
`cic-mcp-session/.mcp.json.tpl`-ben MÁR bevált `"cic-session"` bejegyzés
mintáját követve (host-natív `.venv-host/bin/python` interpreter,
`mcp-server/gateway_server.py` script, `PYTHONPATH` env forwarding) — NE
találj ki új konvenciót, NE módosítsd a meglévő `"cic-graph"` bejegyzést.
...
- [ ] `.mcp.json.tpl`-ben ÚJ `"cic-gateway"` bejegyzés, a `"cic-graph"`
      bejegyzés érintetlen, a `"cic-session"` minta követve
...
Push a `feature/cic-mcp-gateway-mcp-write-confinement-fix-001` branch-re,
KIZÁRÓLAG a `cic-mcp-gateway` célrepóban (a `cic-mcp-factory` saját
